...
examples = {
    ...
    56: Example(  # useful ex26
        ...
        # I_zones=[[-0.1, 0.1]] * 3,
        # U_zones=[[-0.18, -0.15]] * 3,
        D_zones=Zones('box', low=[-0.2, -0.2,-0.2], up=[0.2, 0.2,0.2]),
        I_zones=Zones('box', low=[-0.1, -0.1,-0.1], up=[0.1, 0.1,0.1]),
        U_zones=Zones('box', low=[-0.18, -0.18,-0.18], up=[-0.15, -0.15,-0.15]),
        f=[lambda x, u: -x[1] + u[0],
           lambda x, u: -x[2],
    ...
    515:Example(
            ...
            I_zones=Zones(shape='ball', center=[0, 0, 0], r=1 ** 2),
            G_zones=Zones(shape='ball', center=[0, 0, 0], r=0.5 ** 2),
            U_zones=Zones(shape='ball', center=[0, 0, 0], r=2 ** 2, inner=False),
            f=[lambda x, u: x[2] + 8 * x[1],
               lambda x, u: -x[1] + x[2],
               lambda x, u: -x[2] - x[0] ** 2 + u[0],  ##--+
               ],
